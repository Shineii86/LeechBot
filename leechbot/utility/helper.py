...
from leechbot.utility.variables import BOT, MSG, BotTimes, Messages, Paths, save_uploaded_cache
...
def is_duplicate(file_path: str) -> bool:
    """Check if file already uploaded based on hash and size."""
    if not ospath.exists(file_path):
        return False
    file_size = ospath.getsize(file_path)
    file_hash = compute_file_hash(file_path)
    key = (file_hash, file_size)
    if key in BOT.UPLOADED_FILES:
        return True
    BOT.UPLOADED_FILES.add(key)
    save_uploaded_cache()
    return False
...
# =============================================================================
# Speed and ETA Calculation (with optional speed limit)
# =============================================================================
def speedETA(start_time: datetime, done: int, total: int, limit: int = 0):
...
# =============================================================================
# Message Deleter (with auto-delete setting)
# =============================================================================
...
async def send_settings(client, message, msg_id: int, is_command: bool):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton("Thumb 🖼️", callback_data="thumb")
        ],
        [
        ...
        [
            InlineKeyboardButton(f"⏳ Auto-Delete: {'ON' if BOT.Setting.auto_delete else 'OFF'}", callback_data="autodelete")
        ],
        [
            InlineKeyboardButton("Speed Limit ⚡", callback_data="speed_limit")
        ],
        [
...
# =============================================================================
# Status Bar Update (with speed limit awareness)
# =============================================================================
